...
enum ManagedPreferencesImport {
    private static let domain = "de.twocent.notary"
    private static let managedPreferencesDirectory = URL(fileURLWithPath: "/Library/Managed Preferences", isDirectory: true)

    static func importCurrentNotaryValues(consoleUser: String?) throws -> (values: [String: String], source: URL) {
        let candidates = candidateURLs(consoleUser: consoleUser)
        ...
        for url in candidates {
            ...
            do {
                let values = try MobileConfigImport.importNotaryValues(from: url)
                return (values, url)
            } catch {
                continue
            }
        ...
        let searched = checkedPaths.isEmpty
            ? candidates.map(\.path).joined(separator: "\n")
            : checkedPaths.joined(separator: "\n")
        throw NSError(
            domain: "NotaryConfigurator",
            code: 404,
            userInfo: [NSLocalizedDescriptionKey: "No readable Notary managed preferences were found.\n\nSearched:\n\(searched)"]
        )
    }

    private static func candidateURLs(consoleUser: String?) -> [URL] {
        var urls: [URL] = [
            managedPreferencesDirectory.appendingPathComponent("\(domain).plist"),
        ]

        if let consoleUser, !consoleUser.isEmpty {
            urls.append(
                managedPreferencesDirectory
                    .appendingPathComponent(consoleUser, isDirectory: true)
        ...
        urls.append(
            managedPreferencesDirectory
                .appendingPathComponent("_unknown", isDirectory: true)
                .appendingPathComponent("\(domain).plist")
        )
