...
enum NotaryGUI {
    ...
    static func showReportWindow(logger: HardenLogger) {
        ...
        delegate.installReportURLHandler()
        app.finishLaunching()
    ...
    fileprivate static func isCurrentUserAdmin() -> Bool {
...
private final class NotaryReportWindowDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        installReportOpenNotificationObserver()
    ...
    func applicationWillTerminate(_ notification: Notification) {
        ...
        DistributedNotificationCenter.default().removeObserver(self, name: NotaryReportOpenRequest.notificationName, object: nil)
        beginShutdown()
    }
    ...
    @objc func refreshContent() {
        ...
        do {
            ...
            complianceMetric?.value = report.publicComplianceState.displayTitle
    ...
    @objc func openConfiguratorPlaceholder() {
        ...
        window.makeKeyAndOrderFront(nil)
        delegate.offerCurrentManagedConfigurationImport()
        ensureAppPresentation()
    }
    ...
    @objc private func handleGetURLEvent(_ event: NSAppleEventDescriptor, withReplyEvent replyEvent: NSAppleEventDescriptor) {
        guard let urlString = event.paramDescriptor(forKeyword: keyDirectObject)?.stringValue,
              ...
              url.scheme?.lowercased() == "notary"
        else {
            return
        }

        if url.host?.lowercased() == "report" || url.path == "/report" {
            openOrFocusReportWindow()
        }
    }

    func installReportURLHandler() {
    ...
    private func installReportOpenNotificationObserver() {
        DistributedNotificationCenter.default().addObserver(
            self,
            selector: #selector(handleReportOpenNotification(_:)),
            name: NotaryReportOpenRequest.notificationName,
            object: nil
        )
    }

    @objc private func handleReportOpenNotification(_ notification: Notification) {
        openOrFocusReportWindow()
    }

    private func openOrFocusReportWindow() {
        isShuttingDown = false

        let reportWindow: NSWindow
        if let existing = window {
            reportWindow = existing
        } else {
            reportWindow = buildWindow(frame: NSRect(x: 0, y: 0, width: 1260, height: 840))
        }

        refreshContent()
        if reportWindow.isMiniaturized {
            reportWindow.deminiaturize(nil)
        }
...
private final class NotaryConfiguratorWindowDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, NSOutlineViewDataSource, NSOutlineViewDelegate, NSTextFieldDelegate, NSSearchFieldDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        reloadSchema()
        window.makeKeyAndOrderFront(nil)
        offerCurrentManagedConfigurationImport()
        ensureAppPresentation()
    }
    ...
    func offerCurrentManagedConfigurationImport() {
        guard !nodes.isEmpty else { return }

        let alert = NSAlert()
        alert.alertStyle = .informational
        alert.messageText = "Aktuelle Konfiguration laden?"
        alert.informativeText = "Soll der Configurator die auf diesem Mac verwaltete Notary-Konfiguration aus /Library/Managed Preferences laden?"
        ...
        guard response == .alertFirstButtonReturn else {
            importStatusField?.stringValue = "Started with schema defaults."
            importStatusField?.textColor = .secondaryLabelColor
            return
        }

        loadCurrentManagedConfiguration()
    }
    ...
    private func loadCurrentManagedConfiguration() {
        do {
            let imported = try ManagedPreferencesImport.importCurrentNotaryValues(consoleUser: ManagedPrefs.consoleUser())
            applyImportedValues(imported.values)
            importStatusField?.stringValue = "Loaded \(imported.values.count) values from \(imported.source.path)."
            importStatusField?.textColor = .secondaryLabelColor
            updateDetails(for: selectedNode, error: nil)
        } catch {
            importStatusField?.stringValue = "Managed Preferences import failed: \(error.localizedDescription)"
            importStatusField?.textColor = .systemRed
        }
    }

    private func applyImportedValues(_ values: [String: String]) {
