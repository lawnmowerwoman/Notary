...
private func postSharingdPreferenceChange(user: String, logger: HardenLogger) -> Bool {
    guard let uid = PerUserPreferences.uid(for: user) else {
        logger.error("[AirDropAutoDisable] Could not post sharingd notification; no uid found for \(user).")
        return false
    }

    let notificationName = "com.apple.sharingd.preferencesChanged"
    ...
    guard let result = try? Shell.run("/bin/launchctl", args, timeout: 5, logger: logger) else {
        logger.error("[AirDropAutoDisable] Failed to post sharingd notification for \(user).")
        return false
    }

    if result.code == 0 && !result.didTimeout {
        logger.info("[AirDropAutoDisable] Posted sharingd notification for \(user).")
        return true
    } else {
        logger.error("[AirDropAutoDisable] notifyutil failed for \(user): \(Shell.failureDetailsForReport(command: "/bin/launchctl", args: args, result: result, timeout: 5))")
        return false
    }
}

func restartSharingdIfNeeded(user: String, logger: HardenLogger) {
    guard let uid = PerUserPreferences.uid(for: user) else {
        logger.error("[AirDropAutoDisable] Could not restart sharingd; no uid found for \(user).")
        return
    }
    ...
    guard let result = try? Shell.run("/bin/launchctl", args, timeout: 10, logger: logger) else {
        logger.error("[AirDropAutoDisable] Failed to restart sharingd for \(user).")
        return
    }

    if result.code == 0 && !result.didTimeout {
        logger.info("[AirDropAutoDisable] sharingd restarted for \(user) as fallback.")
    } else {
        logger.error("[AirDropAutoDisable] sharingd restart failed for \(user): \(Shell.failureDetailsForReport(command: "/bin/launchctl", args: args, result: result, timeout: 10))")
    }
}

func enforceAirDropDisabled(user: String, logger: HardenLogger) {
    let success = PerUserPreferences.writeDefaultsBool(
        user: user,
        domain: applicationAccessDomain,
        key: airDropKey,
        value: false,
    ...
    guard success else {
        logger.error("[AirDropAutoDisable] Failed to write AirDrop disabled flag for \(user)")
        return
    }

    if !PerUserPreferences.synchronizeDefaults(user: user, domain: applicationAccessDomain, logger: logger) {
        logger.warn("[AirDropAutoDisable] Could not synchronize AirDrop defaults for \(user).")
    }

    let notifiedSharingd = postSharingdPreferenceChange(user: user, logger: logger)
    if !notifiedSharingd {
        restartSharingdIfNeeded(user: user, logger: logger)
    }

    logger.info("[AirDropAutoDisable] AirDrop disabled for user \(user); sharingd refresh \(notifiedSharingd ? "notification sent" : "fallback requested").")
}
