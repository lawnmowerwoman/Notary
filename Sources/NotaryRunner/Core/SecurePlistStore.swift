...
package final class SecurePlistStore<Value: Codable> {
    ...
    private let preservedExternalKeys: Set<String>
    private let rootURL: URL
    private let tmpURLOverride: URL?

    package init(
        logger: PlistStoreLogger? = nil,
        preservedExternalKeys: Set<String> = [],
        rootURL: URL = URL(fileURLWithPath: "/var/db/notary.plist", isDirectory: false),
        tmpURL: URL? = nil
    ) {
        self.logger = logger
        self.preservedExternalKeys = preservedExternalKeys
        self.rootURL = rootURL
        tmpURLOverride = tmpURL
    }

    /// Fallback for non-root (per-UID to avoid collisions/symlink games).
    private var tmpURL: URL {
        if let tmpURLOverride {
    ...
    package func save(_ value: Value) throws {
        ...
        do {
            ...
            data = try dataByPreservingExternalKeys(newData: data, existingURL: url)
        } catch {
    ...
    private func dataByPreservingExternalKeys(newData: Data, existingURL: URL) throws -> Data {
        guard !preservedExternalKeys.isEmpty,
        ...
        guard var newPlist = try PropertyListSerialization.propertyList(from: newData, options: [], format: nil) as? [String: Any],
              let existingPlist = try PropertyListSerialization.propertyList(from: existingData, options: [], format: nil) as? [String: Any] else {
            return newData
        }

        var didPreserve = false
        for key in preservedExternalKeys where newPlist[key] == nil {
            if let value = existingPlist[key] {
                newPlist[key] = value
                didPreserve = true
            }
        }

        guard didPreserve else { return newData }
